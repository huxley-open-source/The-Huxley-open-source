...
<head>
    ...
    <script type="text/javascript">
        var limit = 8, name="", licenseSearchInputTimeOut, lastIndex;

        function getLicense(index){
            var offset = index * limit;
            $.ajax({
                url: hx.util.url('license/search'),
                data: 'offset=' + offset + '&max='+limit + '&name=' + name + '&t=5',
                    ...
                    $.each(data.licenses, function(i, license) {
                        if (license.profile !== null) {
                            toAppend += '<div class="th-license-user-box">' +
                                    ...
                                    '</ul>'+
                                    '</div>'+
                                    '<a href="javascript:void(0)" onclick="removeLicense(\'' + license.hash +'\')"><span class="th-close">×</span></a>'+
                                    '</div></div>';
                        } else {
                            toAppend += '<div class="th-license-user-box">' +
                                    '<div class="user-box">'+
                    ...
                    if(!(totalIndexUserSearch != -1 && (event.keyCode==40||event.keyCode==38||event.keyCode==13))){
                        if ($('#input-email').val() != '') {
                            $.ajax({
                                ...
                                data: 'offset=0&max=5&name=' + $('#input-email').val() ,
                                beforeSend: huxley.showLoading(),
                                dataType: 'json',
                                success: function(data) {
                            ...
                            $('#user-search-list').show().fadeIn('fast');
                        } else {
                            $('#user-search-list').hide().fadeOut('fast')
        ...
        $(function() {
            $('#manage-pendencies').hide();
            $('#create-teacher').on('click', validateEmail);
            getLicense(0);
...
<body>
<div class="th-tabbed-pane"><ul><li> <a class="th-tabbed-pane-tab selected" id="teacher-tab" href="javascript:addTeacherTab()"><g:message code="license.add.admin"/></a></li><li><a class="th-tabbed-pane-tab" id="pendencies-tab" href="javascript:managePendenciesTab()">Gerenciar Pendências</a></li></ul></div>
<div class="box tabbed" id="add-teacher">

    <span class="th-right muted th-license-available"><small id="licenses-available-create"></small></span>
    <h3><g:message code="license.create.admin.license"/></h3>
    <div class="th-clear"></div>
    <div>
        <input type="text" name="name" placeholder="${message(code: 'verbosity.add.user.by.email')}" autocomplete="off" style="width: 62%;" class="th-input" id="input-email"  /><a class="btn btn-success" id="create-teacher"><g:message code="license.add"/></a>
    </div>
    <span id="email-msg"></span>
    <div class="th-search-container">
        <div id="user-search-list" class="th-search-result-container"></div>
    </div>
    <h3>Instituição <span class="ui-custom-select" style="display:table;margin-top:-5px;">
...
<div class="box"><!-- Search box -->
    <div class="th-license-list-header">
        <h3><g:message code="license.list.admin"/></h3>
...
<script type="text/template" id="pendency-item">
    <tr>
        <td>
            <div class="row-fluid">
                <div class="span8">
                    {{ userCreated.name + " - " + institution.name }}
                </div>
                ...
                success: function (data) {
                    ...
                    $.each(data, function (i, pendency) {
                        ...
                        dom.delegate('buttom.js-accept:not([disabled])', 'click', function () {
                            $.get('/huxley/pendency/acceptPendency', {id: pendency.id}).done(function () {
                                getLicense(lastIndex);
                                fetchingPendencies();
                            });
                        });
                        dom.delegate('buttom.js-reject:not([disabled])', 'click', function () {
                            ...
                            $(template).delegate('button.js-reject-confirm', 'click', function () {
                                $.post('/huxley/pendency/rejectPendency', {id: pendency.id, message: $(template).find('textarea').val()}).done(function () {
                                    getLicense(lastIndex);
                                    fetchingPendencies();
                                });
