    ...
    <script type="text/javascript">
        ...
        function getLicense(index){
            ...
            $.ajax({
                ...
                data: 'offset=' + offset + '&max='+limit + '&name=' + name + '&t=3',
        ...
        function updateTotal() {
            $.ajax({
                ...
                dataType: 'json',
                success: function(data) {
                    var msgContainer = $('#licenses-available-create, #licenses-available-pendencies'), createTeacherButtom = $('#create-teacher'), acceptButtons = $('.js-accept');
                    msgContainer.empty();
...
<body>
<div class="th-tabbed-pane"><ul><li> <a class="th-tabbed-pane-tab selected" id="teacher-tab" href="javascript:addTeacherTab()"><g:message code="license.add.teacher"/></a></li><li><a class="th-tabbed-pane-tab" id="pendencies-tab" href="javascript:managePendenciesTab()">Gerenciar Pendências</a></li></ul></div>
...
<div class="box"><!-- Search box -->
    <div class="th-license-list-header">
        <h3><g:message code="license.list.teacher"/></h3>
                        ...
                        $.each(data, function (i, pendency) {
                            ...
                            dom.delegate('buttom.js-reject:not([disabled])', 'click', function () {
                                ...
                                $(template).delegate('button.js-reject-confirm', 'click', function () {
                                    $.post('/huxley/pendency/rejectPendency', {id: pendency.id, message: $(template).find('textarea').val()}).done(
                                            function () {
                                                getLicense(lastIndex);
                                                fetchingPendencies();
                                            }
